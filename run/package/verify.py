...
def checkSingleBench(bench_name, num_of_sym_bound, value_assigned_for_prediction, cache_config):
    ...
    trace_path = "../data/traced_data/"
    # read symbolic RI
    sym_ri_src_only        = read_sym_ri(sym_ri_path + bench_name + ".ri." + cache_config + ".src_only")
    sym_ri_src_enhanced    = read_sym_ri(sym_ri_path + bench_name + ".ri." + cache_config + ".src_snk")
    sym_ri_src_snk         = read_sym_ri(sym_ri_path + bench_name + ".ri." + cache_config + ".src_snk_plus")
    sym_isnk               = read_sym_isnk(sym_ri_path + bench_name, cache_config + ".src_snk_plus")
    ...
    predicted_histogram_src_enhanced = evaluate(sym_ri_src_enhanced, sym_ibound_src_only, value_assigned_for_prediction, "src_snk")
    ...
    predicted_histogram_src_snk = evaluate([sym_ri_src_snk, sym_isnk], sym_ibound_src_only, value_assigned_for_prediction, "src_snk_plus")
    ...
    traced_file = trace_path + cache_config + "/" + bench_name + "/" + bench_name + "_" + cache_config 
    traced_histogram = readTraceFile(traced_file, value_assigned_for_prediction, cache_config)
    ...
    plotHistoAllBars(predicted_histogram_src_only, predicted_histogram_src_enhanced, predicted_histogram_src_snk, traced_histogram, bench_name, cache_config, value_assigned_for_prediction_str)    
...
'''
def verify():
    num_of_symbolic_bounds = {"bit_reversal" : 1}
