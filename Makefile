...
bench_2para= stencil_tiled #heat_3d trangle adi atax bicg convolution_2d correlation covariance deriche gramschmidt heat_3d jacobi_1d jacobi_2d seidel_2d symm syr2d syrk trmm
bench_3para= #doitgen convolution_3d doitgen fdtd_2d gemm  
bench_4para= #2mm
bench_5para= #3mm

inputoutputGenTarget= -FORMATSRCITERPOS -FORMATSRCITERPOSSNK -FORMATSRCITERPOSSNKITERPOS -FORMATSRCSHAPE

train_size=4 6 8 12 16 20
...
inputoutput_gen:
	...
	$(foreach name, $(bench_1para), $(BIN_DIR)/inputOutputGen -NAME $(name) -SIZES ${train_size} -NUMOFLOOPBOUNDS 1 -SAMPLINGRATE ${SRATE} ${inputoutputGenTarget} -CACHECONFIG ELM;)
	$(foreach name, $(bench_1para), $(BIN_DIR)/inputOutputGen -NAME $(name) -SIZES ${train_size} -NUMOFLOOPBOUNDS 1 -SAMPLINGRATE ${SRATE} ${inputoutputGenTarget} -CACHECONFIG CLS32_DS8;)
	$(foreach name, $(bench_2para), $(BIN_DIR)/inputOutputGen -NAME $(name) -SIZES ${train_size} -NUMOFLOOPBOUNDS 2 -SAMPLINGRATE ${SRATE} ${inputoutputGenTarget} -CACHECONFIG ELM;)
	$(foreach name, $(bench_2para), $(BIN_DIR)/inputOutputGen -NAME $(name) -SIZES ${train_size} -NUMOFLOOPBOUNDS 2 -SAMPLINGRATE ${SRATE} ${inputoutputGenTarget} -CACHECONFIG CLS32_DS8;)
